// Framer-Motion, Layout Group & Animate Presence
...
const ExpandableBox = ({ title, hoveredText, isButton, buttonLabel, unHoveredText, durationTime }: Props) => {
	...
	return (
      {/* LayoutGroup & layout allows to animate everything inside as one */}
		<LayoutGroup>
			<MotionFlex
			...
			>
				...
				>
					<AnimatePresence initial={false} mode={"wait"}>
						{/* AnimatePresence is needed for the exit prop to work */}
						{/* <AnimationVariant>
                  {isHovering ? hoveredText : unHoveredText}
               </AnimationVariant> */}
						<MotionBox as={"span"} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: durationTime }} key={isHovering ? "hovering" : "unhovering"}>
							{isHovering ? (
								<Flex className="hoveredContainer" flexDirection={"row"} alignItems={"flex-start"} justifyContent={"space-between"} gap={4}>
									<Flex className="hoveredWrapper" flexDirection={"column"} alignItems={"flex-start"} style={{ textWrap: "balance" }}>
										{title && (
											<H3Header margin={".5rem 0 .25rem 0"} padding={"0px"} textTransform="capitalize">
												{title}
											</H3Header>
										)}
										<Box
										...
										{/* </Flex> */}
										{isButton && (
											<Button size={"sm"} borderRadius={"full"} variant={colorMode === "light" ? "primary" : "accent"} rightIcon={<FaArrowRight />} textTransform={"capitalize"} margin={"1rem 0 "}>
												{buttonLabel}
											</Button>
										)}
									</Flex>
								...
								<Flex flexDirection={"column"}>{unHoveredText}</Flex>
							)}
						</MotionBox>
					</AnimatePresence>
				</MotionBox>
